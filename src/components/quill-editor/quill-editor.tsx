...
import { File, Folder, Workspace } from "@/lib/mongoose/types";
import React, {
  useCallback,
  useEffect,
  useMemo,
...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  const deleteFileHandler = async () => {
    if (dirType === "file") {
      ...
      dispatch({
        type: "DELETE_FILE",
        payload: { fileId, folderId: fId, workspaceId: wsId },
      });
  ...
  useEffect(() => {
    ...
    const fetchInformation = async () => {
      if (dirType === "file") {
        ...
        // Temporarily use the passed dirDetails instead
        ...
        const fileDir = selectedDir as File;
        dispatch({
          type: "UPDATE_FILE",
          payload: {
            file: { data: selectedDir.data },
            fileId,
            folderId: fileDir.folderId,
            workspaceId,
          },
        });
      }

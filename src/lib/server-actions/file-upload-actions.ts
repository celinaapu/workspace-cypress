...
// ✅ No disk, no filesystem — works on any serverless platform
...
export async function uploadWorkspaceLogo(formData: FormData) {
  ...
  if (!file || !workspaceId) return { success: false, error: 'Missing file or workspace ID' };
  ...
  try {
    ...
    return { success: false, error: 'Logo upload failed' };
  }
}
...
export async function uploadBanner(formData: FormData) {
  ...
  if (!file || !id) return { success: false, error: 'Missing file or ID' };
  if (!file.type.startsWith('image/')) return { success: false, error: 'Only image files are allowed' };
  if (file.size > 5 * 1024 * 1024) return { success: false, error: 'File size must be less than 5MB' };
